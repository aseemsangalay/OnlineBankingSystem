...
const UpdateAccount = ({ account, setAccount }) => {
    ...
    return (
        <div className='login_container'>
            {/* <p style={{'color': '#313131'}}>{JSON.stringify(account, null, 2)}</p> */}
            <div className='login_bag'>
                <Link to="/">
                    <img className={"login_logo"} src={obs_logo} alt="logo" />
                </Link>
                <h2 style={{'color': '#313131', 'marginTop': '0'}}>Update an Account</h2>
                ...
                <button className='custom_btn' onClick={() => addAccount()} >UPDATE ACCOUNT</button>
            </div>
        </div>
    )
}
